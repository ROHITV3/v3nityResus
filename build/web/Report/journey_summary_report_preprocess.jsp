...
<%  UserProperties userProperties = (UserProperties) request.getAttribute("properties");
    ...
    //          HX PASTEOVER
    //String inputEndDate = dateTimeFormatter.format(today);
    String inputEndDate = "15/11/2019";
    //String inputStartDate = dateTimeFormatter.format(today);
    String inputStartDate = "11/11/2019";
    //          HX PASTEOVER END
%>
        ...
        <script type="text/javascript">
            ...
            $(document).ready(function()
            {
                ...
                var tbl = $("#resultTable").DataTable(
                {
                    ...
                    paging: true,
                    ajax: {
                        url: 'JourneySummaryPreprocessController?type=system&action=get',
                        data: function(d)
                        {
                            ...
                            //HX COMMENT
//                            d.date = $("#dateTimePicker-history-Ending-date").val();
//                            d.day = $("#numDays").val();
                            //HX COMMENT END
                            
                            //          HX ADD ON
                            d.end= $("#dateTimePicker-history-Ending-date").val();
                            d.start= $("#dateTimePicker-history-Starting-date").val();
                            //          HX ADD ON END
                        },
                        beforeSend: function()
                        {
                ...
                $("#dateTimePicker-history-Ending-date").AnyTime_picker({format: "%d/%m/%Y", earliest: new Date(2019,09,10,0,0,0), latest: d});
                
                $("#dateTimePicker-history-Starting-date").AnyTime_picker({format: "%d/%m/%Y", earliest: new Date(2019,09,10,0,0,0), latest: d});

                $("#downloadSummary").click(function()
                {                    
                    if ($("#dateTimePicker-history-Starting-date").val())
                    {
                        window.open("JourneySummaryPreprocessController?type=system&action=csv&start=" + $("#dateTimePicker-history-Starting-date").val() + "&end=" + $("#dateTimePicker-history-Ending-date").val(), "_blank", "toolbar=no,status=no,scrollbars=no,menubar=no,height=0,width=0,resizeable=no");
                    }
                    else
                    {
...
//          HX PASTEOVER
            function dispose()
            {
                $("#dateTimePicker-history-Starting-date").AnyTime_noPicker();
                $("#dateTimePicker-history-Ending-date").AnyTime_noPicker();
            }
//          HX PASTEOVER
...
//          HX PASTEOVER
            function resetDate()
            {
                ...
                $("#dateTimePicker-history-Ending-date").val(resetEndDate);
                
                var resetStartDate = '<%=inputStartDate%>';
                $("#dateTimePicker-history-Starting-date").val(resetStartDate);
            }
//          HX PASTEOVER END
...
<!--                        <select id="numDays" name="numDays" style="width: 48px">
                    ...
                    <div class="input-control text" data-role="input">
                        ...
                        <input id="dateTimePicker-history-Starting-date" type="text" placeholder="<%=userProperties.getLanguage("selectStartDate")%>" autocomplete="on">
                        <button class="button helper-button clear"><span class="mif-cross"></span></button>
                    </div> to 
                    <div class="input-control text" data-role="input">
                        <span class="mif-calendar prepend-icon"></span>
                        <input id="dateTimePicker-history-Ending-date" type="text" placeholder="<%=userProperties.getLanguage("selectEndDate")%>" autocomplete="on">
                        <button class="button helper-button clear"><span class="mif-cross"></span></button>
                    </div>
                    <!--HX PASTEOVER END-->
                </div>
