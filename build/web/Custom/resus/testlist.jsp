...
<html>
    <head>
        ...
        <script type="text/javascript">
            ...
            var csv_${namespace} = new csvDownload('TestListController?lib=${lib}&type=${type}&action=view', 'V3NITY');
                ...
                var tbl = $('#${namespace}-result-table').DataTable(
                    {
                        ...
                        responsive: true,
                        ajax: {
                            url: 'TestListController?lib=${lib}&type=${type}&format=json&action=view',
                            type: 'POST',
                            data: function(d)
                            {
            ...
            function cellEditCallback_${namespace}(cell, row, oldValue)
            {
                ...
                if (cell.data() !== oldValue)
                {
                    ...
                    $.ajax({
                        type: 'POST',
                        url: 'TestListController?lib=${lib}&type=${type}&action=edit',
                        data: dataEdit,
            ...
            $("#${namespace}-tool-button-delete").click(function()
            {
                ...
                if (data.length > 0)
                {
                    ...
                    if (c === true)
                    {
                        ...
                        $.ajax({
                            type: 'POST',
                            url: 'TestListController?lib=${lib}&type=${type}&action=delete',
                            data: {
                                id: ids
                            },
                            beforeSend: function()
            ...
            function deviceSync_${namespace}()
            {
                $.ajax({
                    type: 'POST',
                    url: 'TestListController?lib=${lib}&type=${type}&action=sync',
            ...
            function ${namespace}_downloadFileEmail()
            {
                ...
                if (prefix !== '')  // must select at least 1 asset...
                {
                    ...
                    $.ajax({
                        type: 'POST',
                        url: 'TestListController?lib=${lib}&type=${type}&action=email&format=csv',
                        data: options,
                        async: true,
                        success: function(data)
                        {
            ...
            %>
                ...
                {
                    $('#${namespace}-sub-type').load('TestListController?lib=${lib}&type=<%=subType%>&filter=' + id + '&foreignKey=<%=subTypeForeignKey%>');
                }
            <%
                }
            %>
            ...
            function ${namespace}_saveForm()
            {
                if (listForm_${namespace}.save())
                {
                    ...
                    $.ajax({
                        type: 'POST',
                        url: 'TestListController?lib=${lib}&type=${type}&action=' + action + '&id=' + id,
                        data: $('#${namespace}-form-dialog-data').serialize(),
                        beforeSend: function()
                        {
                            $('#${namespace}-button-save').prop("disabled", true);
            ...
            function getData_${namespace}(id)
            {
                ...
                $.ajax({
                    type: 'POST',
                    url: 'TestListController?lib=${lib}&type=${type}&action=data',
                    data: {
                        id: id
                    },
            ...
            function influencerChanged()
            {
                ...
                $.ajax({
                    type: 'POST',
                    url: 'TestListController?lib=${lib}&type=${type}&action=influence',
                    data: {
                        index: metadataIndex,
                        foreign: this.value
                    },
            ...
            $("#${namespace}-tool-button-delete-filter").click(function()
            {
                if (customFilterQuery_${namespace}.length > 0 && totalRecords_${namespace} > 0)
                {
                    ...
                    if (c === true)
                    {
                        $.ajax({
                            type: 'POST',
                            url: 'TestListController?lib=${lib}&type=${type}&action=deleteByFilter',
                            data: {
        ...
        if (cols == "" || cols == null)
          cols = 1
        var output = '<table id="quotationGrid" border="2" cellspacing="1" cellpadding="8" class="table">'

        function createTable(rows, cols) {
          for (var i = 1; i <= rows; i++) {
            output = output + '<tr id="Row' + i + '">'
            for (var j = 1; j <= cols; j++) {
              output = output + '<td>' + '<input class="input" type="text" placeholder="input data">' + '</td>'
            }
            output = output + '</tr>'
          }
          output = output + '</table>'
            ...
            <div class="form-dialog">
                ...
                <div class="form-dialog-content">
                    ...
                    <form id=${namespace}-form-dialog-data method="post" action="list.jsp" autocomplete="off">
                        ...
                        <div>
                            <%
                                    pageContext.include("test123.html");
