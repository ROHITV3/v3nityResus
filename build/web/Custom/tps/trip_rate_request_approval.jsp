<%@page import="v3nity.cust.biz.tps.data.TpsTripRequestAndApprovalListForm"%>
...
<html>
    <head>
        ...
        <script type="text/javascript">
            ...
            var csv_${namespace} = new csvDownload('TpsTripRateRequestAndApprovalController?lib=${lib}&type=${type}&action=view', 'V3NITY');
                ...
                var tbl = $('#${namespace}-result-table').DataTable(
                        {
                            ...
                            responsive: true,
                            ajax: {
                                url: 'TpsTripRateRequestAndApprovalController?lib=${lib}&type=${type}&format=json&action=view',
                                type: 'POST',
                                data: function (d)
                                {
                                    ...
                                    d.changestatusflag=changestatusflag;
                                },
                                beforeSend: function ()
                                {
...
view_data=data.data;
            ...
            var trip_rate_table_id = 0;
            var trip_rate_value = 0;
            $("select[name='loading_site_id']").change(function () {
                var unload_value = $("select[name = 'unloading_site_id']").val();
                var material_value= $("select[name = 'material_type_id']").val();
                if (unload_value != "" && unload_value != undefined && material_value != ""  && material_value !=undefined) {
                    $("select[name = 'trip_rate']").val($("select[name='loading_site_id']").val() + '@' + unload_value+'@'+$("select[name='material_type_id']").val());
                    trip_rate_table_id = $("select[name = 'trip_rate'] :selected").attr('name');
                    trip_rate_value = $("select[name = 'trip_rate'] :selected").text();
//                    console.log(trip_rate_value);
                }
            });
            $("select[name='unloading_site_id']").change(function () {
                var load_value = $("select[name = 'loading_site_id']").val();
                  var material_value= $("select[name = 'material_type_id']").val();
                if (load_value != "" && load_value != undefined && material_value != "" && material_value !=undefined) {
                    $("select[name = 'trip_rate']").val(load_value + '@' + $("select[name='unloading_site_id']").val()+'@'+$("select[name='material_type_id']").val());
                    trip_rate_table_id = $("select[name = 'trip_rate'] :selected").attr('name');
                    trip_rate_value = $("select[name = 'trip_rate'] :selected").text();
//                    console.log(trip_rate_value);
                }
            });
    $("select[name='material_type_id']").change(function () {
                var unload_value = $("select[name = 'unloading_site_id']").val();
                var load_value = $("select[name = 'loading_site_id']").val();
                if (unload_value != "" && unload_value != undefined && load_value != "" && load_value != undefined) {
                    $("select[name = 'trip_rate']").val($("select[name='loading_site_id']").val() + '@' + unload_value+'@'+$("select[name='material_type_id']").val());
                    trip_rate_table_id = $("select[name = 'trip_rate'] :selected").attr('name');
                    trip_rate_value = $("select[name = 'trip_rate'] :selected").text();
//                    console.log(trip_rate_value);
...
function readyAllmethod(){
    changestatusflag="ready";
    readyallids="";
 var view_data_temp="";
     for(var i=0;i<view_data.length;i++){
          view_data_temp=view_data[i];
          if(view_data_temp[28]=="New"){
              
              readyallids+=view_data_temp.DT_RowId+","
    
          }
     }
         readyallids=  readyallids.substring(0,readyallids.length-1)
                $.ajax({
                    type: "POST",
                    encType: "multipart/form-data",
                    url: "TpsTripRateRequestAndApprovalController?lib=${lib}&type=${type}&action=ReadyAllStatus&id=" + readyallids + "&status=" + 1,
                    cache: false,
                    contentType: false,
                    beforeSend: function ()
                    {
...
changestatusflag="false";
 var view_data_temp="";
     for(var i=0;i<view_data.length;i++){
          ...
          if(view_data_temp[28]=="Ready" && view_data_temp[19]=="" && view_data_temp[23]==""){
              
              sendallids+=view_data_temp.DT_RowId+","
    
          }
     }
              sendallids=  sendallids.substring(0,sendallids.length-1);
               $.ajax({
                    type: "POST",
                    encType: "multipart/form-data",
                    url: "TpsTripRateRequestAndApprovalController?lib=${lib}&type=${type}&action=sendMail&id=" + sendallids,
                    cache: false,
                    contentType: false,
                    beforeSend: function ()
                    {
...
function commitallmethod(){

changestatusflag="commit";
...
commitallids="";
 ...
 var view_data_temp="";
     for(var i=0;i<view_data.length;i++){
          view_data_temp=view_data[i];
   console.log(view_data_temp)
          if(view_data_temp[28]=="Pending Approval"  && view_data_temp[23]==""){
              
              commitallids+=view_data_temp.DT_RowId+","
    
          }
     }
         commitallids=  commitallids.substring(0,commitallids.length-1)
                $.ajax({
                    type: "POST",
                    encType: "multipart/form-data",
                    url: "TpsTripRateRequestAndApprovalController?lib=${lib}&type=${type}&action=CommitAllStatus&id=" + commitallids + "&status=" + 1,
                    cache: false,
                    contentType: false,
                    beforeSend: function ()
                    {
            ...
            function processReadyStatus(status, id) {

                $.ajax({
                    type: "POST",
                    encType: "multipart/form-data",
                    url: "TpsTripRateRequestAndApprovalController?lib=${lib}&type=${type}&action=changeReadyStatus&id=" + id + "&status=" + status,
                    cache: false,
                    contentType: false,
                    beforeSend: function ()
                    {
            ...
            function processCommitStatus(status, id) {
        $.ajax({
                    type: "POST",
                    encType: "multipart/form-data",
                    url: "TpsTripRateRequestAndApprovalController?lib=${lib}&type=${type}&action=changeCommitStatus&id=" + id + "&status=" + status,
                    cache: false,
                    contentType: false,
                    beforeSend: function ()
                    {
            ...
            function cellEditCallback_${namespace}(cell, row, oldValue)
            {
                ...
                if (cell.data() !== oldValue)
                {
                    ...
                    $.ajax({
                        type: 'POST',
                        url: 'TpsTripRateRequestAndApprovalController?lib=${lib}&type=${type}&action=edit',
                        data: dataEdit,
                        success: function (data)
                        {
            ...
            $("#${namespace}-tool-button-delete").click(function ()
            {
                ...
                if (data.length > 0)
                {
                    ...
                    if (c === true)
                    {
                        ...
                        $.ajax({
                            type: 'POST',
                            url: 'TpsTripRateRequestAndApprovalController?lib=${lib}&type=${type}&action=delete',
                            data: {
                                id: ids
                            },
                            beforeSend: function ()
            ...
            function deviceSync_${namespace}()
            {
                $.ajax({
                    type: 'POST',
                    url: 'TpsTripRateRequestAndApprovalController?lib=${lib}&type=${type}&action=sync',
            ...
            function ${namespace}_downloadFileEmail()
            {
                ...
                if (prefix !== '')  // must select at least 1 asset...
                {
                    ...
                    $.ajax({
                        type: 'POST',
                        url: 'TpsTripRateRequestAndApprovalController?lib=${lib}&type=${type}&action=email&format=csv',
                        data: options,
                        async: true,
                        success: function (data)
                        {
            ...
            %>
                ...
                {
                    $('#${namespace}-sub-type').load('TpsTripRateRequestAndApprovalController?lib=${lib}&type=<%=subType%>&filter=' + id + '&foreignKey=<%=subTypeForeignKey%>');
                }
            <%
                }
            %>
            ...
            function ${namespace}_saveForm()
            {
                ...
                if (listForm_${namespace}.save())
                {
                    ...
                    var ttl_rate= $("input[name='total_rate']").val();
                 
                    if(trip_rate_value!=ttl_rate){
                        $("select[name = 'trip_rate'] :selected").text(ttl_rate)
                    }
                    ...
                    $.ajax({
                        type: 'POST',
                        url: 'TpsTripRateRequestAndApprovalController?lib=${lib}&type=${type}&action=' + action + '&id=' + id + '&trip_rate_table_id=' + trip_rate_table_id + "&trip_rate_value=" + trip_rate_value,
                        data: $('#${namespace}-form-dialog-data').serialize(),
            ...
            function showAdd_${namespace}()
            {
                ...
                $("input[name=trip_rate_id]").parent().parent().css("display", "none");
                clearForm_${namespace}();
                $("select[name=status_id]").val(1);
            ...
            function getData_${namespace}(id)
            {
                ...
                $.ajax({
                    type: 'POST',
                    url: 'TpsTripRateRequestAndApprovalController?lib=${lib}&type=${type}&action=data',
                    data: {
                        id: id
                    },
 ...
 var data=customFilterQuery_${namespace}[0];
  console.log(data);
  if(data.field=="requested_dt"){
      data.value1= data.value1.split(" ")[0];
      data.value1+= " 00:00:00";
        data.value2= data.value2.split(" ")[0];
              data.value2+= " 00:00:00";
  }
  customFilterQuery_${namespace}[0]=data;
                refreshDataTable_${namespace}();
            }
            ...
            function influencerChanged()
            {
                ...
                $.ajax({
                    type: 'POST',
                    url: 'TpsTripRateRequestAndApprovalController?lib=${lib}&type=${type}&action=influence',
                    data: {
                        index: metadataIndex,
                        foreign: this.value
                    },
            ...
            $("#${namespace}-tool-button-delete-filter").click(function ()
            {
                if (customFilterQuery_${namespace}.length > 0 && totalRecords_${namespace} > 0)
                {
                    ...
                    if (c === true)
                    {
                        $.ajax({
                            type: 'POST',
                            url: 'TpsTripRateRequestAndApprovalController?lib=${lib}&type=${type}&action=deleteByFilter',
                            data: {
    ...
    <body>
        ...
        <div class="toolbar" style="margin: 16px 0">
            ...
            <div class="toolbar-section">
                <button class="toolbar-button" style="width: 51px;" type="button" title="Ready All" onclick="readyAllmethod()"><span class="text-light text-small">Ready All</span></button>
            </div>
            <div class="toolbar-section">
                <button class="toolbar-button" style="width: 51px;" type="button" title="Send All" onclick="sendallmethod()"><span class="text-light text-small">Send All</span></button>
            </div>
            <div class="toolbar-section">
                <button class="toolbar-button" style="width: 59px;" type="button" title="Commit All" onclick="commitallmethod()"><span class="text-light text-small">Commit All</span></button>
            </div>
<!--            <div>
        
                <a target="_blank" href="http://localhost:8081/v3nity4/trip_rate_request_for_approval?lib=v3nity.cust.biz.tps.data&type=TpsTripRateRequestForApproval&approval_id=8574">Click to view Trip Rates</a>
                           <a target="_blank" href="http://localhost:8081/v3nity4/trip_rate_request_for_approval?lib=v3nity.cust.biz.tps.data&type=TpsTripRateRequestForApproval&approval_id=8937">Click to view Trip Rates  1</a>
                           <a target="_blank" href="http://localhost:8081/v3nity4/trip_rate_request_for_approval?lib=v3nity.cust.biz.tps.data&type=TpsTripRateRequestForApproval&approval_id=8673">Click to view Trip Rates  2</a>
                           <a target="_blank" href="http://localhost:8081/v3nity4/trip_rate_request_for_approval?lib=v3nity.cust.biz.tps.data&type=TpsTripRateRequestForApproval&approval_id=9784">Click to view Trip Rates  3</a>
...
<!--               <div class="cell" style="    margin: 0;">
               ...
               <div class="input-control text" data-role="input">
                        ...
                        <input id="dateTimePicker-start-requested_dt" class="start" type="text" placeholder="From" value="" readonly="" style="padding-right: 36px;">
                        <button class="button helper-button clear" tabindex="-1" type="button"><span class="mif-cross"></span></button>
                    </div>

                    <script>
                        $("#dateTimePicker-start-requested_dt").AnyTime_picker({
                            format: "%d/%m/%Y"
                        });
                    </script>
                    </div>
                    <div></div>
                                                        <div class="cell" style="    margin: 0;">
                    <label style="margin-right: 4.3%" >To</label>


                    <div class="input-control text" data-role="input" >
                        <span class="mif-calendar prepend-icon"></span>
                        <input id="dateTimePicker-end-ho_original_schedul_time" class="start" type="text" placeholder="To" value="" readonly="" style="padding-right: 36px;">
                        <button class="button helper-button clear" tabindex="-1" type="button"><span class="mif-cross"></span></button>
                    </div>

                    <script>
                        $("#dateTimePicker-end-ho_original_schedul_time").AnyTime_picker({
                            format: "%d/%m/%Y"
                        });
                    </script>
                </div>-->
            ...
            <div class="form-dialog">
                ...
                <div class="form-dialog-content">
                    ...
                    <form id=${namespace}-form-dialog-data method="post" action="list.jsp" autocomplete="off">
                        <div class="grid">
                            <%
                                TpsTripRequestAndApprovalListForm listForm = new TpsTripRequestAndApprovalListForm();
