...
<html>
    <head>
        ...
        <script type="text/javascript">
            ...
            var csv_${namespace} = new csvDownload('JobAssetController?lib=${lib}&type=${type}&action=view', 'V3NITY');
                ...
                var tbl = $('#${namespace}-result-table').DataTable(
                    {
                        ...
                        responsive: true,
                        ajax: {
                            url: 'JobAssetController?lib=${lib}&type=${type}&format=json&action=view',
                            type: 'POST',
                            data: function(d)
                            {
            ...
            function cellEditCallback_${namespace}(cell, row, oldValue)
            {
                ...
                if (cell.data() !== oldValue)
                {
                    ...
                    $.ajax({
                        type: 'POST',
                        url: 'JobAssetController?lib=${lib}&type=${type}&action=edit',
                        data: dataEdit,
            ...
            $("#${namespace}-tool-button-delete").click(function()
            {
                ...
                if (data.length > 0)
                {
                    ...
                    if (c === true)
                    {
                        ...
                        $.ajax({
                            type: 'POST',
                            url: 'JobAssetController?lib=${lib}&type=${type}&action=delete',
                            data: {
                                id: ids
                            },
                            beforeSend: function()
            ...
            function deviceSync_${namespace}()
            {
                $.ajax({
                    type: 'POST',
                    url: 'JobAssetController?lib=${lib}&type=${type}&action=sync',
            ...
            function ${namespace}_downloadFileEmail()
            {
                ...
                if (prefix !== '')  // must select at least 1 asset...
                {
                    ...
                    $.ajax({
                        type: 'POST',
                        url: 'JobAssetController?lib=${lib}&type=${type}&action=email&format=csv',
                        data: options,
                        async: true,
                        success: function(data)
                        {
            ...
            %>
                ...
                {
                    $('#${namespace}-sub-type').load('JobAssetController?lib=${lib}&type=<%=subType%>&filter=' + id + '&foreignKey=<%=subTypeForeignKey%>');
                }
            <%
                }
            %>
            ...
            function ${namespace}_saveForm()
            {
             debugger;
                if (listForm_${namespace}.save())
                {
                    ...
                    $.ajax({
                        type: 'POST',
                        url: 'JobAssetController?lib=${lib}&type=${type}&action=' + action + '&id=' + id,
                        data: $('#${namespace}-form-dialog-data').serialize(),
                        beforeSend: function()
                        {
                            $('#${namespace}-button-save').prop("disabled", true);
            ...
            function getData_${namespace}(id)
            {
                ...
                $.ajax({
                    type: 'POST',
                    url: 'JobAssetController?lib=${lib}&type=${type}&action=data',
                    data: {
                        id: id
                    },
            ...
            $("#${namespace}-tool-button-delete-filter").click(function()
            {
                if (customFilterQuery_${namespace}.length > 0 && totalRecords_${namespace} > 0)
                {
                    ...
                    if (c === true)
                    {
                        $.ajax({
                            type: 'POST',
                            url: 'JobAssetController?lib=${lib}&type=${type}&action=deleteByFilter',
                            data: {
            ...
            function downloadQrPDFOld()
            {
                var pdfdialog = window.open("JobAssetController?lib=${lib}&type=${type}&action=downloadQrPdfOld", "_blank", "toolbar=no,status=no,scrollbars=no,menubar=no,height=" + screen.height + ",width=" + screen.width + ",resizeable=no");
            ...
            {
                var pdfdialog = window.open("JobAssetController?lib=${lib}&type=${type}&action=downloadQrPdf", "_blank", "toolbar=no,status=no,scrollbars=no,menubar=no,height=" + screen.height + ",width=" + screen.width + ",resizeable=no");
    ...
    <body>
        ...
        <div class="toolbar" style="margin: 16px 0">
            ...
            <div class="toolbar-section">
                ...
                <button class="toolbar-button" type="button" title="Download Old QR as PDF" onclick="downloadQrPDFOld()"><span class="text-light text-small">OLD QR</span></button>
                <button class="toolbar-button" type="button" title="Download New QR as PDF" onclick="downloadQrPDF()"><span class="text-light text-small">NEW QR</span></button>
