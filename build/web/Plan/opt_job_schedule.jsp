        ...
        <script type="text/javascript">
            ...
            function saveNewLocation() {
                $.ajax({
                    ...
                    {
                        closeLocForm();
                        dialog('Success', 'Location has been saved', 'success');
                        // function in _opt_param
                        repopulateLocation($("#locationName").val());
            ...
            function repopulateLocation()
            {
                $.ajax({
                    ...
                    success: function (data)
                    {
                        ...
                        htmlFrom += "<option value=''>- From -</option>";
                        htmlTo += "<option value=''>- To -</option>";

                        for (var i = 0; i < locations.length; i++)
                        {
                            var loc = locations[i];
                            htmlFrom += "<option value='" + loc.id + "'" + ">" + loc.location + "</option>";
                            htmlTo += "<option value='" + loc.id + "'" + ">" + loc.location + "</option>";
                        }
                        
                        $('select[id=inputFromName').replaceWith("<select id=\"inputFromName\">" + htmlFrom + "</select>");
                        
                        $('select[id=inputToName').replaceWith("<select id=\"inputToName\">" + htmlTo + "</select>");
            ...
            function getBaseLocation()
            {
                $.ajax({
                    ...
                    success: function (data)
                    {
                        ...
                        {
                            var locId = locations[0].id;
                            $("#inputFromName").val(''+locId).change();
                        }
                    },
                    error: function ()
                    {
...
//new end
            ...
            <%=(data.hasSelection()) ? "style: 'multi'" : ""%>
                ...
                //by shubham
                $('select[name=customer_list_id]').on('change', function ()
                {
                    $.ajax({
                        type: 'POST',
                        url: 'JobScheduleController',
                        data: {
                            type: 'system',
                            action: 'getcusinfo',
                            cusId: $("select[name=customer_list_id]").val()
                        },
                        beforeSend: function ()
                        {
                            $("#cusAddr").html('');
                            $("#cusInfo #cusPhone").html('');
                            $("#cusInfo #cusEmail").html('');
                        },
                        success: function (data)
                        {
                            if (data.address)
                                $("#cusAddr").html('Address: ' + data.address);
                            if (data.phone)
                                $("#cusInfo #cusPhone").html('Phone: ' + data.phone);
                            if (data.email)
                                $("#cusInfo #cusEmail").html('Email: ' + data.email);
                        },
                        error: function ()
                        {
                            dialog('Error', 'System has encountered an error', 'alert');
...
//                }
            ...
            {
                var geotagCheck = "E";
                var docdialog = window.open("JobScheduleController?type=plan&action=doc&id=" + id);
                document.getElementById("downloadPdfButton" + id).style.color = 'white';
                document.getElementById("downloadPdfButton" + id).style.backgroundColor = 'green';
            }
            ...
            {
                if (parseInt($('#inputToName').val(),10) > 0)
                {
                    var prefStartArr = '';
                    var prefEndArr = '';
                    ...
                    for (var i = 0 ; i < 20 ; i++)
                    {
                        var aPrefStart = $('#dateTimePicker-pref-start-'+i).val();
                        var aPrefEnd = $('#dateTimePicker-pref-end-'+i).val();
                        ...
                        if (aPrefStart)
                        {
                            ...
                            prefStartArr += aPrefStart;
                            prefEndArr += aPrefEnd;
                        }
                    }

                    var quantity = $('#optQuantity').val();

                    var duration = $('#optDuration').val();

                    var fromId = $('#inputFromName').val();

                    var toId = $('#inputToName').val();

                    var properties = '';

                    $('[id^="property"]').each(function (index, elem)
                    {
                        if($(elem).is(":checked"))
                        {
                            var strArr = $(elem).attr("id").split('_');
                    ...
                    /*
                     *  Some additional checks required here:
                     *  1) capacity and duration must be > 0
                     *  2) fromId <> toId
                     *  3) prefStartArr < prefEndArr
                     *  4) properties not blank
                     */
                    ...
                    $.ajax({
                        ...
                        data: {
                            type: 'system',
                            action: 'saveoptimization',
                            customerId: <%=customerId%>,
                            quantity: quantity,
                            duration: duration,
                            fromId: fromId,
                            toId: toId,
                            prefStartArr: prefStartArr,
                            prefEndArr: prefEndArr,
...
//                            }
            ...
            function dropMarkerCoordinate(lat, lon, road)
            {
                ...
                $('#addLocationText').val(location_road);//new
    ...
    <body>
        ...
        %>
        <div data-role="dialog" id="location-dialog" class="large" data-close-button="true" data-background="bg-white" style="z-index: 1073 !important"><!--new added z-index-->
            <div id ="container">
                    ...
                    <form id=form-dialog-data method="post" action="list.jsp" autocomplete="off">
                        ...
                        <div class="grid panel">
                            ...
                            <div class="row cells2">
                                ...
                                <%--by shubham--%>  
                                <div class="cell">
                                    <div id="cusInfo" class="input-control text full-size" style="margin: 1.010rem 0;color:green">
                                        <div id="cusAddr"></div>
                                        <div id="cusPhone"></div>
                                        <div id="cusEmail"></div>
                                    </div>
                        ...
                        <div class="grid panel">
                            <%
                                    pageContext.include("opt_job_schedule_optimization_parameters.jsp");
                                } catch (Exception e) {
        ...
        <!--new-->
        <div data-role="dialog" id="add-location-dialog" class="location" data-background="bg-white" data-overlay="true" data-overlay-color="op-dark" style="width: 40% !important; height: 30% !important;z-index: 1072 !important;">
            <div class="form-dialog">
                <div class="grid">
                    <div class="cell">
                        <label>Name</label>
                        <div class="input-control text full-size">
                            <input id="locationName" type="text" size="50" placeholder="Name" value="" style="width: 86%;">
                        </div>
                    </div>
                    <div class="cell">
                        <h4 class="text-light align-left"><%=userProperties.getLanguage("assignLocation")%></h4>
                        <div id="locationSearch" class="input-control text full-size" data-role="input">
                            <input id="addLocationText" type="text" size="50" placeholder="<%=userProperties.getLanguage("search")%>">
                            <button class="button" id="location" value="" onclick="openMapDialog();
                                    dropMarker(true);"><span class="mif-location"></span>
                            </button>
                        </div>
                    </div>
                </div>
            ...
            <div class="form-dialog-control">
                <button id=button-saJobScheduleListFormve-optimization type="button" class="button primary" onclick="saveNewLocation()"><%=userProperties.getLanguage("save")%></button>
                <button id=button-cancel-optimization type="button" class="button" onclick="closeLocForm()"><%=userProperties.getLanguage("cancel")%></button>
            </div>
        </div>
        <!--new end-->
    </body>
