...
<html>
    <head>
        ...
        <script type="text/javascript">
            ...
            var csv_${namespace} = new csvDownload('LocationFmsController?lib=${lib}&type=${type}&action=view', 'V3NITY');
                ...
                var tbl = $('#${namespace}-result-table').DataTable(
                    {
                        ...
                        responsive: true,
                        ajax: {
                            url: 'LocationFmsController?lib=${lib}&type=${type}&format=json&action=view',
                            type: 'POST',
                            data: function(d)
                            {
            ...
            function cellEditCallback_${namespace}(cell, row, oldValue)
            {
                ...
                if (cell.data() !== oldValue)
                {
                    ...
                    $.ajax({
                        type: 'POST',
                        url: 'LocationFmsController?lib=${lib}&type=${type}&action=edit',
                        data: dataEdit,
            ...
            $("#${namespace}-tool-button-delete").click(function()
            {
                ...
                if (data.length > 0)
                {
                    ...
                    if (c === true)
                    {
                        ...
                        $.ajax({
                            type: 'POST',
                            url: 'LocationFmsController?lib=${lib}&type=${type}&action=delete',
                            data: {
                                id: ids
                            },
                            beforeSend: function()
            ...
            function deviceSync_${namespace}()
            {
                $.ajax({
                    type: 'POST',
                    url: 'LocationFmsController?lib=${lib}&type=${type}&action=sync',
            ...
            function ${namespace}_downloadFileEmail()
            {
                ...
                if (prefix !== '')  // must select at least 1 asset...
                {
                    ...
                    $.ajax({
                        type: 'POST',
                        url: 'LocationFmsController?lib=${lib}&type=${type}&action=email&format=csv',
                        data: options,
                        async: true,
                        success: function(data)
                        {
            ...
            %>
                ...
                {
                    $('#${namespace}-sub-type').load('LocationFmsController?lib=${lib}&type=<%=subType%>&filter=' + id + '&foreignKey=<%=subTypeForeignKey%>');
                }
            <%
                }
            %>
            ...
            function ${namespace}_saveForm()
            {
                if (listForm_${namespace}.save())
                {
                    ...
                    $.ajax({
                        type: 'POST',
                        url: 'LocationFmsController?lib=${lib}&type=${type}&action=' + action + '&id=' + id,
                        data: $('#${namespace}-form-dialog-data').serialize(),
                        beforeSend: function()
                        {
                            $('#${namespace}-button-save').prop("disabled", true);
            ...
            function getData_${namespace}(id)
            {
                ...
                $.ajax({
                    type: 'POST',
                    url: 'LocationFmsController?lib=${lib}&type=${type}&action=data',
                    data: {
                        id: id
                    },
            ...
            $("#${namespace}-tool-button-delete-filter").click(function()
            {
                if (customFilterQuery_${namespace}.length > 0 && totalRecords_${namespace} > 0)
                {
                    ...
                    if (c === true)
                    {
                        $.ajax({
                            type: 'POST',
                            url: 'LocationFmsController?lib=${lib}&type=${type}&action=deleteByFilter',
                            data: {
