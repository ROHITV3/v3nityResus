...
<%@page import="v3nity.cust.biz.data.ExpenseManagerListForm"%>
...
//    boolean accessJobStatusChart = listProperties.canAccessView(listProperties.getOperations(Resource.DASHBOARD_JOB_STATUS));
...
<html>
    ...
    text-shadow: 0 0 black;
        ...
        <script type="text/javascript">
            ...
            var csv_${namespace} = new csvDownload('ManagerApprovalController?lib=${lib}&type=${type}&action=view', 'V3NITY');
                ...
                var tbl = $('#${namespace}-result-table').DataTable(
                        {
                            ...
                            responsive: true,
                            ajax: {
                                url: 'ManagerApprovalController?lib=${lib}&type=${type}&format=json&action=view',
                                type: 'POST',
                                data: function (d)
                                {
...
//                alert(status +"  drivers=="+driverData);
                window.open("ManagerApprovalController?lib=${lib}&type=${type}&format=csv&action=download&reportdate=" + $("#dateTimePicker-history-start-date").val() + "&reportEndDate=" + $("#dateTimePicker-history-end-date").val() + "&status=" + status + "&driverIdData=" + driverData, "toolbar=no,status=no,scrollbars=no,menubar=no,height=0,width=0,resizeable=no");
                });
            ...
            function cellEditCallback_${namespace}(cell, row, oldValue)
            {
                ...
                if (cell.data() !== oldValue)
                {
                    ...
                    $.ajax({
                        type: 'POST',
                        url: 'ManagerApprovalController?lib=${lib}&type=${type}&action=edit',
                        data: dataEdit,
            ...
            $("#${namespace}-tool-button-delete").click(function ()
            {
                ...
                if (data.length > 0)
                {
                    ...
                    if (c === true)
                    {
                        ...
                        $.ajax({
                            type: 'POST',
                            url: 'ManagerApprovalController?lib=${lib}&type=${type}&action=delete',
                            data: {
                                id: ids
                            },
                            beforeSend: function ()
            ...
            function deviceSync_${namespace}()
            {
                $.ajax({
                    type: 'POST',
                    url: 'ManagerApprovalController?lib=${lib}&type=${type}&action=sync',
...
//                });
            ...
            function ${namespace}_downloadFileEmail()
            {
                ...
                if (prefix !== '')  // must select at least 1 asset...
                {
                    ...
                    $.ajax({
                        type: 'POST',
                        url: 'ManagerApprovalController?lib=${lib}&type=${type}&action=email&format=csv',
                        data: options,
                        async: true,
                        success: function (data)
                        {
            ...
            %>
                ...
                {
                    $('#${namespace}-sub-type').load('ManagerApprovalController?lib=${lib}&type=<%=subType%>&filter=' + id + '&foreignKey=<%=subTypeForeignKey%>');
                }
            <%
                }
            %>
            ...
            function ${namespace}_saveForm()
            {
                if (listForm_${namespace}.save())
                {
                    ...
                    $.ajax({
                        type: 'POST',
                        url: 'ManagerApprovalController?lib=${lib}&type=${type}&action=' + action + '&id=' + id + '&manageStatus=' + manageStatus,
                        data: $('#${namespace}-form-dialog-data').serialize(),
                        beforeSend: function ()
                        {
                            $('#${namespace}-button-save').prop("disabled", true);
            ...
            function getData_${namespace}(id)
            {
                ...
                $.ajax({
                    type: 'POST',
                    url: 'ManagerApprovalController?lib=${lib}&type=${type}&action=data',
                    data: {
                        id: id
                    },
            ...
            function populateForm_${namespace}(result)
            {
                ...
                $.each(result.data, function (i, field)
                {
                    ...
                    {
                        $('input[name=' + field.name + ']').prop("disabled", true);
            ...
            $("#${namespace}-tool-button-delete-filter").click(function ()
            {
                if (customFilterQuery_${namespace}.length > 0 && totalRecords_${namespace} > 0)
                {
                    ...
                    if (c === true)
                    {
                        $.ajax({
                            type: 'POST',
                            url: 'ManagerApprovalController?lib=${lib}&type=${type}&action=deleteByFilter',
                            data: {
            ...
            <div class="form-dialog">
                ...
                <div class="form-dialog-content">
                    ...
                    <form id=${namespace}-form-dialog-data method="post" action="list.jsp" autocomplete="off">
                        <div class="grid">
                            <%
                                ExpenseManagerListForm listForm = new ExpenseManagerListForm();
