        ...
        <script type="text/javascript">
            ...
            function populateOptTimetable(selectedDate, driverIndex, jobIndex)
            {
                ...
                for (var i = 0 ; i < timetableDialogData.length; i++)
                {
                    ...
                    {
                        driverData = timetableDialogData[i].drivers;
                        var unassigned = timetableDialogData[i].unassigned;
                        break;
                    }
                }
                var items = new Array();
                var groups = new Array();
                var calendar = selectedDate.split('-');
                var index = 0;
                    ...
                    for (var i = 0; i < driverData.length; i++) 
                    {
                        if(i & 1 === 1)
                        {
                            var group = 
                            {
                                ...
                                content: driverData[i].resource_name,
                        ...
                        {
                            var startTime = jobs[j].start.split(':');
                            var endTime = jobs[j].end.split(':');
//                            if(i & 1 === 1)
                        if(jobs[j].attribute !== 178)
                        {
                            var item =
                            {
                                id: index++,
                                content: jobs[j].description,
                                title: jobs[j].location,
                                start: new Date(calendar[2], calendar[1], calendar[0], startTime[0], startTime[1]),
                                end:  new Date(calendar[2], calendar[1], calendar[0], endTime[0], endTime[1]),
                                group: driverData[i].driver_id,
                                className: 'orange',
                                editable: true,
                                selectable: true
                            };
                    ...
                    if (typeof unassigned !== 'undefined')
                    {
                        ...
                        //New group for unassigned jobs
                        for (var i = 0 ; i < unassigned.length ; i++)
                        {
                            var startTime = unassigned[i].timePrefStart.split(':');
                            var endTime = unassigned[i].timePrefEnd.split(':');

                            var item =
                            {
                                id: index++,
                                content: unassigned[i].description,
                                title: unassigned[i].location,
                                start: new Date(calendar[2], calendar[1], calendar[0], startTime[0], startTime[1]),
                                end: new Date(calendar[2], calendar[1], calendar[0], endTime[0], endTime[1]),
                                group: 'Unassigned Jobs',
                                subgroup: index,
                                className: 'red',
                                editable: true,
                                selectable: true
                      ...
                      var container = document.getElementById('opt-timetable');
                      ...
                      {    
                          if(typeof properties.group !== 'undefined' && properties.group !== 'Unassigned Jobs')
                          {
                              changeSelectedResource(properties.group, true);
                            }
                          if(typeof properties.item !== 'null')
                            {
                            var ind = 0;
                            
                            for (var i = 0; i < driverData.length; i++) 
                            {
                                for(var j = 0; j < driverData[i].jobs.length; j++)
                                {
                                    if(properties.item === ind)
                                    {
                                        zoomResourceMap(driverData[i].driver_id, j);
                                        changeSummaryContent(driverData[i].driver_id, j);
                            }
                                    ind ++;
                        }
                    }
                        ...
                        //changeSummaryContent(item.group.content, item.jobInd);
...
//                        drivers.push(driverData[i].driver_id);
                        ...
                        var html = '<a href="#"><div class="timetable-side-label" '
                                    + 'id="driver_label_' + driverData[i].driver_id + '" '
//                                    + 'onclick="clickSideBar(\'' + driverData[i].driver_id + '\');"><div>'
                                    + '<h4 class="text-light">' + driverData[i].resource_name + '</h4>'
...
//                    $('#opt-timetable').show();
        ...
        function selectOptimizationEngine()
        {
            ...
            else
            {
                html += '<div class="input-control radio big-check" style="margin-top:30px;">';
                html += '<label class="container" style="margin-right: 20px">';
                html += '<input type="radio" name="optimization_engine" id="optimization_engine0" value="1"/>';
                html += '<span class="checkmark"></span>&nbsp; Default Optimization Engine';
                html += '</label></div>'; 
            }
            
            document.getElementById('optimization_table_check').innerHTML = html;
            document.getElementById('optimization_engine').innerHTML = html;
        }
        ...
        <div data-role='dialog' id="optimization-option-dialog" class="small" data-close-button="false" data-background="bg-white" data-overlay="true" data-overlay-color="op-dark">
            <div class="form-dialog">
                ...
                <div class="form-dialog-content" style="margin-top: 110px;">
                   ...
                   <div class="row cells1" style="margin-top:50px;">
                    <label class="text-bold">Optimization Engine</label>
                    <div class="help-tag">
                        <p>Select the optimization engine to use.</p>
                    </div>
                        <div class="cell" id="optimization_table_check">
<!--                            <div class="input-control radio big-check" style="margin-top:30px;">
                                <label class="container">
                                <input type="radio" name="optimization_type" id="optimize_distance"/>
                                <span class="checkmark"></span>&nbsp; Optimize Distance
                                </label>
                                <label class="container" style="margin-left:300px;">
                                <input type="radio" name="optimization_type" id="optimize_skillset"/>
                                <span class="checkmark" ></span>&nbsp; Optimize Skill Set
                                </label>
                            </div>-->
                        </div>
                    </div>
