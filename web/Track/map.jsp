...
<%
    ...
    assetTreeView.setIdentifier("asset");
    ...
    /*
     * gets user-specific zoom / refresh settings...
     */

    int userId = userProperties.getUser().getId();
    int userCustomerId = userProperties.getInt("customer_id");
...
<html>
    <head>
        ...
        <script type="text/javascript" src="js/v3nity-track.js"></script>
        <script type="text/javascript" src="js/jquery.marquee.min.js"></script>
        <script>

            var map, _live, _history, assetLayer, locationLayer, historyLayer, geoFenceLayer, autoRefresh, autoZoom, showEta, historyAssetId, historyAssetLabel, mapPadding, webAlert, webTraffic, trafficNews, locationSearch, enableTraffic, enableHistoryLabel;
            ...
            var currentCameraPlaybackTimestamp = null;
            var livePanelUpdateInterval = null;
            var assetEventList = {};
            var assetColorList = {};
            var locationSearchETA;
            ...
            function initMap()
            {
                ...
                locationSearchETA = new AddressSearchBox('locationSearchETA', 1000, locationCallbackETA, <%=(locale.getCountry().equals("SG"))%>);

                locationSearchETA.enable();
            ...
            <% }%>
                ...
                _live = new v3nityLive('MapController', refreshCallback, errorCallback, refreshElapsed);
            ...
            function onPlayback(event, slick, currentSlide)
            {
                ...
                currentCameraPlaybackTimestamp = record.timestamp;
            }
                    ...
                    else
                    {
                        switch (event)
                        {
                            ...
                            default:
                                ...
                                if (speed === 0)
                                {
                                    imgHTML = '<div class="map-icon-marker small nohover stop"></div>';
                                }
                                else
                                {
            ...
            {
                $('[id^=asset-detail]').html('<span class="fg-red">Try again</span>');
            }
            ...
            function refreshCallback(result)
            {
                setTimeout(function()
                {
                    $('#tree-view-asset').find(':checkbox').removeAttr('disabled');
                }, 2000);
                ...
                if (livePanelUpdateInterval !== null)
                {
                    clearInterval(livePanelUpdateInterval);
                }

                /*
                 * we will make the side panel update in an interval. this is to increase performance and not to choke the map...
                 */
                livePanelUpdateInterval = setInterval(function()
                {
                    /*
                     * we will do a few html updates in one interval...
                     */
                    for (var i = 0; i < 10; i++)
                    {
                        /*
                         * stop the interval because we have finished updating...
                         */
                        if (index >= result.data.length)
                        {
                            clearInterval(livePanelUpdateInterval);

                            livePanelUpdateInterval = null;
                        ...
                        if (data.custom)
                        {
                            ...
                            {
                                case 'ignitionOff':
                                    markerHTML = '<div class="map-icon-marker-custom small" style="background-image: url(\'img/' + data.icons.marker_ignitionoff + '\')"></div>';

                                    break;

                                case 'standingTimeLimit':
                                    markerHTML = '<div class="map-icon-marker-custom small" style="background-image: url(\'img/' + data.icons.marker_standing + '\')"></div>';
                                ...
                                default:
                                    ...
                                    {
                                        markerHTML = '<div class="map-icon-marker-custom small" style="background-image: url(\'img/' + data.icons.marker_moving + '\')"></div>';
                                    }
                            }
                        }
                        else
                        {
                            switch (data.event)
                            {
                                ...
                                case 'immobilise':
                                    markerHTML = '<div class="map-icon-marker small nohover immobilise"></div>';
                        ...
                        $('[id^=asset-detail-' + data.id + '-]').filter('[data-checked=true]').each(function()
                        {
                            var details = $(this);
                             ...
                             */
                            details.attr('data-event', data.event);
                            ...
                            details.html('<blockquote class="text-small uppercase">' +
                                ...
                                ((data.temperature !== '0.0' && data.temperature !== 0 && data.temperature !== 'null') ? data.temperature + '&#176;C&emsp;' : '') +
                                ((data.sectemperature !== '0.0' && data.sectemperature !== 0 && data.sectemperature !== 'null') ? data.sectemperature + '&#176;C&emsp;' : '') +
                                ((data.temperature3 !== '0.0' && data.temperature3 !== 0 && data.temperature3 !== 'null') ? data.temperature3 + '&#176;C&emsp;' : '') +
//                                data.temperature + '&#176;C&emsp;' +
//                                data.sectemperature + '&#176;C&emsp;' +
//                                temp3 + '<br>' +
                                ((data.mainPowerVoltage !== '0' && data.mainPowerVoltage !== 0 && data.mainPowerVoltage !== 'null') ? data.mainPowerVoltage + 'V&emsp;' : '') +
                                ((data.fuelLevel !== '0' && data.fuelLevel !== 0 && data.fuelLevel !== 'null') ? data.fuelLevel + 'litres Full&emsp;' : '') + voltageBreakLine +
                                ((data.rfid) ? data.rfid + '<br>' : '') +
                                ((data.road) ? data.road + '<br>' : '') +
                                ((data.immobiliser === true) ? ((data.immobilised) ? 'IMMOBILISED<br>' : 'MOBILISED<br>') : '') +
//                                '<br>' +
//                            data.rfid + '<br>' +
//                            data.road + '<br>' +
                                ...
                                ((data.camera === true && data.camera_vendor === 0) ? '<br>Camera Live<br>' : '') +
                                ((data.camera === true && data.camera_vendor === 0) ? '<button class="button mini-button" onclick="openCameraDialog(\'' + data.camera_all + '\')">ALL</button>&emsp;' : '') +
                                ((data.camera === true && data.camera_vendor === 0) ? '<button class="button mini-button" onclick="openCameraDialog(\'' + data.camera_cam1 + '\')">1</button>&emsp;' : '') +
                                ((data.camera === true && data.camera_vendor === 0) ? '<button class="button mini-button" onclick="openCameraDialog(\'' + data.camera_cam2 + '\')">2</button>&emsp;' : '') +
                                ((data.camera === true && data.camera_vendor === 0) ? '<button class="button mini-button" onclick="openCameraDialog(\'' + data.camera_cam3 + '\')">3</button>&emsp;' : '') +
                                ((data.camera === true && data.camera_vendor === 0) ? '<button class="button mini-button" onclick="openCameraDialog(\'' + data.camera_cam4 + '\')">4</button>&emsp;' : '') +
                                ((data.camera === true && data.camera_vendor === 0) ? '<br>Camera History<br>' : '') +
                                ((data.camera === true && data.camera_vendor === 0) ? '<button class="button mini-button" onclick="openCameraHistoryDialog(\'' + data.camera_id + '\')"><span class="mif-history"></span></button>&emsp;' : '') +
                                ((data.camera === true && data.camera_vendor === 1) ? '<button class="button mini-button" onclick="openTechLoreCameraDialog(\'' + data.techlore_serial + '\', ' + data.techlore_cameras + ', 1)"><span class="mif-camera"></span></button>&emsp;' : '') +
                                ((data.camera === true && data.camera_vendor === 1) ? '<br>Playback Cameras<br>' : '') +
                                ((data.camera === true && data.camera_vendor === 1) ? addTechloreCameraPlaybackButtons(data.techlore_serial, data.techlore_cameras, 1) : '') +
                                ((data.camera === true && data.camera_vendor === 2) ? '<button class="button mini-button" onclick="openTechLoreCameraDialog(\'' + data.techlore_serial + '\', ' + data.techlore_cameras + ', 2)"><span class="mif-camera"></span></button>&emsp;' : '') +
                                ((data.camera === true && data.camera_vendor === 2) ? '<br>Playback Cameras<br>' : '') +
                                ((data.camera === true && data.camera_vendor === 2) ? addTechloreCameraPlaybackButtons(data.techlore_serial, data.techlore_cameras, 2) : '') +
                                ((data.camera === true && data.camera_vendor === 3) ? '<button class="button mini-button" onclick="openV3nityCameraDialog(\'' + data.dvr_serial + '\')"><span class="mif-camera"></span></button>&emsp;' : '') +
                                '</blockquote>');
                        });
                ...
                for (var i = 0; i < result.data.length; i++)
                {
                    ...
                    if (assetColorList[data.id] === undefined)
                    {
                        $('[id^=asset-detail-' + data.id + '-]').filter('[data-checked=true]').each(function()
                        {
                            var details = $(this);
                            ...
                            if (data.labelColor === undefined & details.attr('data-checked') === 'true')
                            {
                                assetColorList[data.id] = data.labelColor = details.attr('data-group-color');
                            }
                    ...
                    {
                        data.labelColor = assetColorList[data.id];
                    }
                    ...
                    assetEventList[data.id] = eventName.replace(/ /g, '');

                    var newLabel = data.label.replace('\'', '\\\'');

                    var popupHTML = '<div class="uppercase">' +
                        '<h4>' + data.label + ' ' + ((data.model !== null && data.model !== 'null' && data.model !== "") ? '(' + data.model + ')' : '') + '</h4>' +
                        ((data.driver !== null && data.driver !== 'null') ? data.driver + '<br>' : '') +
                        data.timestamp + '<br>' +
                        ((data.isMobileApp === true) ? '' : eventName) + '<br>' +
                        data.rfid + '<br>' +
                        ...
                        ((data.temperature !== '0.0' && data.temperature !== 0 && data.temperature !== 'null') ? data.temperature + '&#176;C&emsp;' : '') +
                        ((data.sectemperature !== '0.0' && data.sectemperature !== 0 && data.sectemperature !== 'null') ? data.sectemperature + '&#176;C&emsp;' : '') +
                        ((data.temperature3 !== '0.0' && data.temperature3 !== 0 && data.temperature3 !== 'null') ? data.temperature3 + '&#176;C&emsp;' : '') + '<br>' +
//                        data.temperature + '&#176;C&emsp;' +
//                        data.sectemperature + '&#176;C&emsp;' +
//                        temp3 + '<br>' +
                        data.road + '<br>' +
                        ((data.immobiliser === true) ? ((data.immobilised) ? 'IMMOBILISED' : 'MOBILISED') : '') + '<br>' +
                    ...
                    markers[data.id] = {'lat': data.latitude, 'lng': data.longitude, 'label': data.label, 'popup': popupHTML};

                    var locationInfo = [data.latitude, data.longitude, data.headingValue, data.speed, data.timestamp, data.event, data.label, popupHTML, data.id, limitWidth, data.labelColor, data.custom, data.icons, labelOn];
...
//                }
            ...
            function toggleAssetDetail(assetId, element, checked)
            {
                ...
                if (checked)
                {
                    ...
                    {
                        //element.innerHTML = '<span class="mif-spinner3 mif-ani-spin"></span>';    // cause performance drop...
                    }
...
//                                map1.removeLastLocationMarker(assetId);
            ...
            function onAssetTreeViewClicked(treeview, parent, children, checked)
            {
                ...
                if (!parent.hasClass('node'))
                {
                    ...
                    var detail = parent.children('[id^=asset-detail-' + assetId + '-]')[0];
                ...
                for (i = 0; i < children.length; i++)
                {
                    ...
                    var assetId = child.data('asset-id');

                    var detail = child.children('[id^=asset-detail-' + assetId + '-]')[0];
            ...
            function onTreeviewCheckboxClicked(treeview, parent, children, checked)
            {
                ...
                if (treeview === 'tree-view-asset')
                {
                    ...
                    {
                        $('#tree-view-asset').find(':checkbox').attr('disabled', 'disabled');
                    }
                }
...
//                    }, 1500);
            ...
            function checkAssetTreeview(assetArray)
            {
                ...
                $('#tree-view-asset').find('[data-name=asset]').find('[type="checkbox"]:checked').each(function()
                {
                ...
                {
                    var asset = $('#tree-view-asset').find('#' + assetArray[i]);
                    asset.attr('data-checked', true).css('display', 'block');
                    asset.parent().find('[type="checkbox"]').prop('checked', true);

                    asset.parent().parent().parent().find('[name="asset-type"][type="checkbox"]').prop('checked', true);

                    asset.parents('li').parents('li').find('[name="asset-group"][type="checkbox"]').prop('checked', true);
                }
            }
            ...
            function toggleHistoryMenu(show)
            {
                ...
                if (!show)
                {
                    ...
                    currentCameraPlaybackTimestamp = null;
                }
                else
                {
                    charm.animate({
                            ...
                            if (data.expired === undefined)
                            {
                                if (data.result === true)
                                {
                                    ...
                                    for (var i = 0; i < assets.length; i++)
                                    {
                                        ...
                                        var event = assetEventList[assets[i].id];
            ...
            {
                if ('${defaultPage}' === 'MapController')
                {
                    load ('DashboardController');
                }
                else if ('${defaultPage}' !== 'none')
                {
                    load('${defaultPage}');
                }
                else
            ...
            function locationCallbackETA(address, lng, lat)
            {
                map.clearDirectionRenderer();
                ...
                var originLng = $('#origin').data('lon');

                getETA(originLat, originLng, lat, lng);
            }

            function getETA(origLat, origLng, destLat, destLng)
            {
                map.getETA(origLat, origLng, destLat, destLng, getETACallback);

                map.drawRoute(origLat, origLng, destLat, destLng);
            }

            function getETACallback(data)
            {
                if(data.eta)
                {
                    $('#etaVal').text(data.eta);
                }
                else
                {
                    $('#etaVal').text("can't find right now");
                }

                if(data.distance)
                {
                    $('#distanceVal').text(data.distance);
                }
                else
                {
                    $('#distanceVal').text("can't find right now");
                }

            }

            function toggleSearchMenuETA()
            {
                var charm = $("#etaCharmSearch");
                if (charm.data('hidden') === undefined)
                {
                    charm.data('hidden', true);
                    charm.css('left', -320);
            ...
            function resetOriginDetails(label, lat, lon)
            {
                var etaHeader = $('#etaHeaderText');

                etaHeader.text(label);
            ...
            function saveAutoZoom()
            {
                var autoZoomSetting = document.getElementById('checkboxAutoZoom').checked;
            ...
            {
                var autoRefreshSetting = document.getElementById('checkboxAutoRefresh').checked;
            ...
            function saveMapEta()
            {
                var mapEtaSetting = document.getElementById('checkboxEta').checked;
            ...
            function getUserMapSettings()
            {
                $.ajax({
                    ...
                    success: function(data)
                    {
                        if (data.MapAutoZoom)
                        {
                            if (data.MapAutoZoom == "0") $("#checkboxAutoZoom").click();
                        }

                        if (data.MapAutoRefresh)
                        {
                            if (data.MapAutoRefresh == "0") $("#checkboxAutoRefresh").click();
                        }

                        if (data.MapEta)
                        {
                            if (data.MapEta == "1") $("#checkboxEta").click();
                        }
                    },
                    error: function()
                    {
    ...
    <body>
        <section>
            ...
            <div class="charm left-side padding10 v3-bg-blue fg-white" id="etaCharmSearch" style="width: 240px;left: 320px;height: 200px;">
                <button class="square-button bg-transparent fg-white no-border place-right small-button" onclick="toggleSearchMenuETA()"><span class="mif-cross"></span></button>
                <h5 class="text-light" id="etaHeaderText"></h5>
                <div class="content padding10 bg-white" style="position: absolute; top: 0; bottom: 0; left: 0px; margin: 54px 0 0 0; width: 100%; overflow-y: auto;">
                    <span style="color: #645f59">ETA for Destination</span>
                    <div id="locationSearchETA" class="input-control text full-size" data-role="input">
                        <input id="locationTextETA" type="text" size="50" placeholder="<%=userProperties.getLanguage("search")%>">
                        <button class="button helper-button clear"><span class="mif-cross"></span></button>
                    </div>
                    <div id="matches-address-eta"></div>
                    <div id="results-address-eta" class="content" style="color: #fff; position: absolute; top: 0; bottom: 0; margin: 168px 0 8px 0; width: 300px; overflow-y: auto"></div>
                    <table id="etaTable" style="width: 220px"><tbody><tr><td style="width: 65%; font-size: 14px">ETA</td><td id="etaVal" style="font-size: 12px"></td></tr><tr><td style="width: 65%; font-size: 14px">Distance</td><td id="distanceVal" style="font-size: 12px"></td></tr></tbody></table>
                </div>
            ...
            <div class="charm left-side padding10 v3-bg-darkerGray shadow" id="charmMapSettings" style="width: 320px">
                ...
                <div class="content padding10 v3-bg-darkerGray" style="position: absolute; top: 0; bottom: 0; left: 0px; margin: 84px 0 0 0; width: 100%; overflow-y: auto">
                    ...
                    <div class="setting-block">
                        <span class="sub-header"><%=userProperties.getLanguage("showEta")%></span>
                        <label class="switch place-right">
                            <input id="checkboxEta" type="checkbox" onchange="toggleShowEta()">
                            <span class="check"></span>
                        </label>
                        <h6 class="text-light"><%=userProperties.getLanguage("showEtaHint")%></h6>
                    </div>
                    <div class="setting-block">
        ...
        <section class="full-screen" style="background-color:black;">
            ...
            <div class="legend-dialog" style="position: fixed; top: 412px; right: 8px;">
                ...
                <%
                    ...
                    {
                %>
                <div><div class="map-icon-marker small nohover moving"></div><span class="text-small"> <%=userProperties.getLanguage("moving")%></span></div>
                        <%
                            }
                            ...
                            {
                        %>
                <div><div class="map-icon-marker small nohover stop"></div><span class="text-small"> <%=userProperties.getLanguage("stop")%></span></div>
                        <%
                            }
                            ...
                            {
                        %>
                <div><div class="map-icon-marker small nohover ignitionoff"></div><span class="text-small"> <%=userProperties.getLanguage("ignitionOff")%></span></div>
                        <%
                            }

                            if (legendOption.charAt(3) == '1')
                            {
                        %>
                <div><div class="map-icon-marker small nohover standing"></div><span class="text-small"> <%=userProperties.getLanguage("standing")%></span></div>
                        <%
                            }

                            if (legendOption.charAt(4) == '1')
                            {
                        %>
                <div><div class="map-icon-marker small nohover idling"></div><span class="text-small"> <%=userProperties.getLanguage("idling")%></span></div>
                        <%
                            }

                            if (legendOption.charAt(5) == '1')
                            {
                        %>
                <div><div class="map-icon-marker small nohover immobilise"></div><span class="text-small"> <%=userProperties.getLanguage("immobilised")%></span></div>
                        <%
                            }
                        %>
                        <%
        ...
        <%@ include file="../Track/custom/techlore_live_camera.jsp"%>
        <%@ include file="../Track/custom/techlore_history_camera.jsp"%>
        <%@ include file="../Track/v3nity_live_camera.jsp"%>
    </body>
</html>