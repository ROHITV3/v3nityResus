        ...
        <script type="text/javascript">
            ...
            $(document).ready(function ()
            {
                ...
                $('select[name=quotationType]').on('change', function ()
                {
                    ...
                    //If Selection is 1 or 2, it's normal quoation
                    if (quotationType === '1' || quotationType === '2') {
                        //Check the total amount
                        if (grandTotalAmount <= 5000 && grandTotalAmount >= 0) {
                            //Show Drop Down
                            ...
                            if ($('select[name=selectAuthoriser]').val() === null || $('select[name=selectAuthoriser]').val() === '') {
                                $('select[name=selectAuthoriser]').val(0);
                            }
                        } else if (grandTotalAmount > 5000) {
                            //Hide Drop Down
                    ...
                    //If Selection is 3 or 4, it's sport hub quotation
                    //Only use the drop down to select the autoriser
                    else if (quotationType === '3' || quotationType === '4') {
                        console.log('Show Drop Down');
                        document.getElementById('header7').style.display = "";
                        document.getElementById('header8').style.display = "none";
                    }
                ...
                if (listForm.save())
                {
                    ...
                    if (quotationType === '1' || quotationType === '2') {
                        if (grandTotalAmount > 5000) {
                            //Set auhotiser id to zero if the quotation type is NOT Sport Hub kind (id=1 || id=2) and more than $5000 on grand total
                            authoriserId = 0;
                            console.log('Set Authoriser Id To 0');
                        }
                    }
                    ...
                    if (mandatoryCheck) {
                        ...
                        if (quotationType === '1' || quotationType === '2') {
                            if (grandTotalAmount > 0 && grandTotalAmount <= 5000)
                            {
                                if (authoriserId === '0' || authoriserId === '' || authoriserId === null) {
                                    dialog('Failed', 'Need to assign BD Authoriser (For $5000 and below)', 'alert');
                                    return;
                                }
                            }
                        } else {
                            if (authoriserId === '0' || authoriserId === '' || authoriserId === null) {
                                dialog('Failed', 'Need to assign BD Authoriser Sport Hub Quotation', 'alert');
                                return;
                            }
                        }
