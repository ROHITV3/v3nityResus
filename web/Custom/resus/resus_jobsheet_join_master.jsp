...
<%
    ...
    String type = "ResusJobsheetJoinMaster";

    ListData data = new ResusJobsheetJoinMaster();
                ...
                var tbl = $('#result-table').DataTable(
                        {
                            ...
                            responsive: true,
                            ajax: {
                                url: 'ResusJobsheetJoinMasterListController?lib=<%=lib%>&type=<%=type%>&format=json&action=view',
                                type: 'POST',
                                data: function (d)
                                {
...
//                                }
            ...
            function joinJobSheetButton(id,jobsheetRefNo)
            {
                $('#button-join').data('action', 'checkIfQuotationExist');
                $('#button-join').data('id', id);
                $('#button-join').data('jobsheetRefNo', jobsheetRefNo);
                $('input[name=quotationRefIdName]').val("");
                $('input[name=quotationRefIdName]').show();
                $('#button-joinConfirm').hide();
                $('#button-join').show();
                //Prompt User To key in the Quotation Ref No
                $('#jobsheetJoin-dialog-title').html('Joining Job Sheet No:' + jobsheetRefNo);
                $('#jobsheetJoin-dialog-content').html('Please Key In Quotation Ref No:');
                //$('#signoff-dialog').animate({width: '600px', height: '150px'});
                $('#jobsheetJoin-dialog').data('dialog').open();
            }
            
            function joinJobsheet(){
                if ($('input[name=quotationRefIdName]').val().trim() === ''){
                ...
                else
                {
                    ...
                    $.ajax({
                        ...
                        data: {
                            type: 'plan',
                            action: $('#button-join').data('action'),
                            quotationRefId: $('input[name=quotationRefIdName]').val()
                        },
                        success: function (data)
                        {
                            if (data.result === true) {
                                ...
                                $('#button-joinConfirm').data('id', quotationId);
                                let jobsheetRefNo = $('#button-join').data('jobsheetRefNo');
                                $('input[name=quotationRefIdName]').hide();
                                $('#jobsheetJoin-dialog-content').html('Do you want to join Job Sheet : ' + jobsheetRefNo + ' to Quotation : ' + $('input[name=quotationRefIdName]').val());
                                $('#button-joinConfirm').show();
                                $('#button-join').hide();
                            } else {
            ...
            function confirmJoinJobsheet(){
                let quotationId = $('#button-joinConfirm').data('id');
                let jobSheetId = $('#button-join').data('id');
                //$('#jobsheetJoin-dialog-content').html('Job Sheet ID : ' + jobSheetId + ' To Quotation Id : ' + quotationId);
                $('#jobsheetJoin-dialog-content').html('Job Sheet Joined To Quotation : ' + $('input[name=quotationRefIdName]').val());
                $('#button-joinConfirm').hide();
                
                let userId = <%=userProperties.getUser().getId()%>;
    ...
    <body>
        ...
        <div data-role="dialog" id="jobsheetJoin-dialog" class="padding20" data-close-button="true" data-width="500" data-height="200" data-overlay="true" data-background="bg-lightOlive" data-color="fg-white">
            <h1 id=jobsheetJoin-dialog-title class="text-light"></h1>
            <p id="jobsheetJoin-dialog-content"></p>
            <br>
            <input type="text" id="quotationRefId" name="quotationRefIdName">
            <div class="form-dialog-control">
                <button id=button-join type="button" class="button" onclick="joinJobsheet()">Join</button>
                <button id=button-joinConfirm type="button" class="button" onclick="confirmJoinJobsheet()">Confirm</button>
                <button id=button-joinClose type="button" class="button" onclick="closeJoinJobsheet()">Close</button>
            </div>
        </div>
